...
char* check_number(char constant[])
{
	...
	*/
	...
	if (strstr(constant, "0x") || strstr(constant, "0X")) {
		char* temp = hex_to_decimal(constant);
	...
	// by a decimal point or is not just '0'
		...
		char* temp = octal_to_decimal(constant);
		returnstr = (char*)malloc(strlen(temp) + 1);
		strcpy(returnstr, temp);
	}
	else {
	// Not HEXADECIMAL or OCTAL - no change required
		returnstr = (char*)malloc(strlen(constant) + 1);
...
char * hex_to_decimal(char constant[])
{
	...
	*/
	...
	char* temp = convert_to_decimal(hexstring, HEX);
	returnstr = (char*)malloc(strlen(temp) + 1);
	strcpy(returnstr, temp);
...
int convert_letter(char ch)
{
	...
	*/
	...
	switch(ch)
	{
		case 'a': case 'A':	{ return 10;  }	// hex A||a = 10	
		case 'b': case 'B':	{ return 11;  }	// hex B||b = 11
		case 'c': case 'C':	{ return 12;  }	// hex C||c = 12
		case 'd': case 'D':	{ return 13;  }	// hex D||d = 13
		case 'e': case 'E':	{ return 14;  }	// hex E||e = 14
		case 'f': case 'F':	{ return 15; }	// hex F||f = 15
		default:  			{ return 0;  }
	}
}
