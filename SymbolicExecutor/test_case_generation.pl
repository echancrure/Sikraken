...
write_test_case(Filename, Function_name, Params, Return_value, Return_type) :-
    ...
    open(Filepath_to_main, write, testcase_main_file),
    printf(testcase_main_file, "#include <stdlib.h>\n", []),
    printf(testcase_main_file, "#include <stdio.h>\n", []),
    printf(testcase_main_file, "#include \"%s.c\"\n\n", [Test_suite_name]),
    printf(testcase_main_file, "int main()\n{\n%s\n", [Add_tests_to_suite_string]),
    printf(testcase_main_file, "\tprintf(\"Tests for %s ran successfully\");\n", [Function_name]),
    printf(testcase_main_file, "\treturn EXIT_SUCCESS;\n}\n", []),
    close(testcase_main_file).
